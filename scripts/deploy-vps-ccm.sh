...
_set_env "SENDER_COOLDOWN_DAYS" "7"
_set_env "SMSTOOLS_WEBHOOK_SECRET" "$(openssl rand -base64 32 | tr -dc 'A-Za-z0-9' | head -c 32)"
...
_ask_key "SMSTOOLS_CLIENT_ID"        "smstools X-Client-Id"
_ask_key "SMSTOOLS_CLIENT_SECRET"    "smstools X-Client-Secret"
_ask_key "SMSTOOLS_SENDER"           "smstools numéro/expéditeur (+324...)"
_ask_key "SMSTOOLS_SENDERS"          "smstools pool de numéros, séparés par virgules (optionnel, vide = SMSTOOLS_SENDER)"
_ask_key "TWILIO_WEBHOOK_SIGNING_KEY" "Twilio Webhook Signing Key (voix)"
...
echo -e "${GREEN}╠═══════════════════════════════════════════════════════╣${NC}"
echo -e "${GREEN}║${NC}  Webhook voix à configurer dans Twilio :           ${GREEN}║${NC}"
echo -e "${GREEN}║${NC}   Voice: https://${APP_DOMAIN}/api/v1/webhooks/twilio/voice ${GREEN}║${NC}"
echo -e "${GREEN}║${NC}  Webhook SMS entrant à configurer dans smstools :  ${GREEN}║${NC}"
echo -e "${GREEN}║${NC}   https://${APP_DOMAIN}/api/v1/webhooks/smstools/inbound?token=<SMSTOOLS_WEBHOOK_SECRET> ${GREEN}║${NC}"
